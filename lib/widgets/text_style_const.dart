import 'package:flutter/material.dart';

class TextStyleS {
  static TextStyle s32w700({color, overflow}) => TextStyle(
      fontSize: 32,
      overflow: overflow ?? TextOverflow.ellipsis,
      fontWeight: FontWeight.w700,
      color: color);

  static TextStyle s28w700({color, overflow}) => TextStyle(
      fontSize: 28,
      overflow: overflow ?? TextOverflow.ellipsis,
      fontWeight: FontWeight.w700,
      color: color);

  static TextStyle s24w600({color, overflow}) => TextStyle(
      fontSize: 24,
      overflow: overflow ?? TextOverflow.ellipsis,
      fontWeight: FontWeight.w600,
      fontFamily: 'ProximaNova',
      color: color);

  static TextStyle s22w600({color, overflow}) => TextStyle(
      fontSize: 22,
      overflow: overflow ?? TextOverflow.ellipsis,
      fontWeight: FontWeight.w600,
      color: color);

  static TextStyle s21w400({color, overflow}) => TextStyle(
      fontSize: 21,
      overflow: overflow ?? TextOverflow.ellipsis,
      fontWeight: FontWeight.w400,
      color: color);

  static TextStyle s20w800() => TextStyle(
        color: Colors.black,
        fontSize: 20,
        fontWeight: FontWeight.w800,
        letterSpacing: 0.38,
      );

  static TextStyle s20w700() => TextStyle(
        fontSize: 20,
        fontWeight: FontWeight.w700,
      );

  static TextStyle s18w700({color, overflow}) {
    return TextStyle(
        overflow: overflow,
        fontWeight: FontWeight.w700,
        fontSize: 18,
        letterSpacing: -0.41,
        color: color);
  }

  static TextStyle s18w600({color, overflow}) {
    return TextStyle(
        overflow: overflow,
        fontWeight: FontWeight.w600,
        fontSize: 18,
        letterSpacing: -0.41,
        color: color);
  }

  static TextStyle s18w500({color, overflow}) {
    return TextStyle(
        overflow: overflow,
        fontWeight: FontWeight.w500,
        fontSize: 18,
        letterSpacing: -0.32,
        color: color);
  }

  static TextStyle s17w600({color, overflow}) {
    return TextStyle(
        overflow: overflow,
        fontWeight: FontWeight.w600,
        fontSize: 17,
        letterSpacing: -0.41,
        color: color);
  }

  static TextStyle s17w400({color, overflow}) {
    return TextStyle(
      overflow: overflow,
      fontWeight: FontWeight.w400,
      fontSize: 17,
    );
  }

  static TextStyle s16w700({color, overflow}) {
    return TextStyle(
        overflow: overflow,
        fontSize: 16,
        color: color,
        fontWeight: FontWeight.w700);
  }

  static TextStyle s16w600({color, overflow}) {
    return TextStyle(
        fontWeight: FontWeight.w600,
        fontSize: 16,
        letterSpacing: -0.41,
        overflow: overflow,
        color: color);
  }

  static TextStyle s16w500({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 16,
      overflow: overflow,
      fontWeight: FontWeight.w500,
    );
  }

  static TextStyle s20w500({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 20,
      overflow: overflow,
      fontWeight: FontWeight.w500,
    );
  }

  static TextStyle s20w300({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 20,
      overflow: overflow,
      fontWeight: FontWeight.w300,
    );
  }

  static TextStyle s16w400({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 16,
      overflow: overflow,
      fontWeight: FontWeight.w400,
    );
  }

  static TextStyle s15w400({color, overflow}) {
    return TextStyle(
        color: color,
        fontSize: 15,
        overflow: overflow,
        fontWeight: FontWeight.w400,
        fontFamily: "ProximaNova");
  }

  static TextStyle s15w500({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 15,
      overflow: overflow,
      fontWeight: FontWeight.w500,
    );
  }

  static TextStyle s15w600({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 15,
      overflow: overflow,
      fontWeight: FontWeight.w600,
    );
  }

  static TextStyle s14w700({color, overflow}) {
    return TextStyle(
        fontWeight: FontWeight.w700,
        fontSize: 14,
        overflow: overflow,
        color: color);
  }

  static TextStyle s14w400l({color, overflow}) {
    return TextStyle(
        color: color, fontSize: 14, overflow: overflow, letterSpacing: 0.2);
  }

  static TextStyle s20w600({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 20,
      overflow: overflow,
      fontWeight: FontWeight.w600,
    );
  }

  static TextStyle s14w400({color, overflow, decoration}) {
    return TextStyle(
      color: color,
      fontSize: 14,
      overflow: overflow,
      decoration: decoration,
      fontWeight: FontWeight.w400,
    );
  }

  static TextStyle s14w600({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 14,
      overflow: overflow,
      fontWeight: FontWeight.w600,
    );
  }

  static TextStyle s14w500({color, overflow}) {
    return TextStyle(
      color: color,
      fontSize: 14,
      fontWeight: FontWeight.w500,
      overflow: overflow,
    );
  }

  static TextStyle s13w700({color, overflow}) {
    return TextStyle(
        color: color,
        fontSize: 13,
        fontWeight: FontWeight.w700,
        overflow: overflow);
  }

  static TextStyle s13w500({color, overflow}) {
    return TextStyle(
        color: color,
        fontSize: 13,
        fontWeight: FontWeight.w500,
        overflow: overflow);
  }

  static TextStyle s17w700({color, overflow}) {
    return TextStyle(
        color: color,
        fontSize: 17,
        fontWeight: FontWeight.w700,
        overflow: overflow);
  }

  static TextStyle s13w400({color, overflow}) {
    return TextStyle(
        color: color,
        fontSize: 13,
        overflow: overflow,
        fontWeight: FontWeight.w400);
  }

  static TextStyle s12w700({color, overflow}) {
    return TextStyle(
      overflow: overflow,
      fontSize: 12,
      color: color,
      fontWeight: FontWeight.w700,
    );
  }

  static TextStyle s12w600({color, overflow}) {
    return TextStyle(
      fontSize: 12,
      overflow: overflow,
      color: color,
      fontWeight: FontWeight.w600,
    );
  }

  static TextStyle s12w400({color, overflow}) {
    return TextStyle(
        fontSize: 12,
        overflow: overflow,
        fontWeight: FontWeight.w400,
        color: color);
  }

  static TextStyle s12w300({color, overflow}) {
    return TextStyle(
        fontSize: 12,
        overflow: overflow,
        fontWeight: FontWeight.w300,
        color: color);
  }

  static TextStyle s12w500({color, overflow}) {
    return TextStyle(
        fontSize: 12,
        overflow: overflow,
        fontWeight: FontWeight.w500,
        color: color);
  }

  static TextStyle s11w400({color, overflow}) {
    return TextStyle(
        fontSize: 11,
        overflow: overflow,
        fontWeight: FontWeight.w500,
        color: color);
  }

  static TextStyle s9w600({color, overflow}) {
    return TextStyle(
        fontSize: 9,
        overflow: overflow,
        fontWeight: FontWeight.w600,
        color: color);
  }

  static TextStyle style({color, fontSize, fontWeight}) {
    return TextStyle(
      fontSize: fontSize ?? 14,
      fontWeight: fontWeight ?? FontWeight.w400,
      color: color,
    );
  }
}
